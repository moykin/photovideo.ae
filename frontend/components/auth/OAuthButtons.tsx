...
/**
 * OAuthButtons — кнопки входа через социальные сети
 *
 * v1.0 — initial: Google, Facebook, Apple.
 *         Редиректят на Strapi /api/connect/{provider} → провайдер → /auth/callback.
 *         Strapi сам обрабатывает OAuth flow, фронт только инициирует редирект.
 *
 * Как работает flow:
 *   1. Клик → window.location = STRAPI/api/connect/google
 *   2. Strapi → Google OAuth consent screen
 *   3. Google → Strapi callback
 *   4. Strapi → FRONTEND/auth/callback?access_token=xxx
 *   5. /auth/callback сохраняет токен → dashboard
 *
 * Настройка в Strapi Admin: Settings → Users & Permissions → Providers
 */
...
export function OAuthButtons({ mode = 'signin', primaryOnly = false }: OAuthButtonsProps) {
  ...
  function handleOAuth(provider: string) {
    // Strapi сам делает redirect на провайдера, потом возвращает на /auth/callback
